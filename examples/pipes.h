...
rec {
  hn hpipe(sender: (()) ~> <Sender(s)> a) : <Receiver(s)> a => a {
     ...
     <Receiver::recv() -> receiver> => resume(sender, (), hcopipe(receiver))
  }

  hn hcopipe(receiver: (s) ~> <Receiver(s)> a) : <Sender(s)> a => a {
     ans => ans, // TODO add some drop primitive for `receiver'.
     <Sender::send(s) -> sender> => resume(receiver, s, hpipe(sender))
  }
}
...
fn main() -> () {
  ...
  let receiver: (i64) ~> <Receiver(i64)> () = alloc(print_nats);
